...
def train_language_model_distributed(rank, world_size):
    ...
    def load_dao_teachings():
        ...
        train_corpus = copy(texts) #[:factor]
        random.shuffle(train_corpus)
        random.shuffle(train_corpus)
        random.shuffle(train_corpus)
        random.shuffle(train_corpus)
        eval_corpus = texts[factor:]
        train_corpus = train_corpus * 250
        random.shuffle(train_corpus)
        random.shuffle(train_corpus)
        random.shuffle(train_corpus)
        random.shuffle(train_corpus)
    ...
    print(f"Average length: {avg_train_length}, Max length: {max_train_length}, Min length: {min_train_length}")
